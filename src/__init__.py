	...
	for camera in gopro.get_all_cameras():
		...
		if not exists(catalog_path):
			with open(catalog_path, "w") as f:
				f.write("# 📷 Firmware archives for %s (%s):\n\n\n" % (camera.Name, camera.ModelString))
		...
		if camera.ReleaseDate == get_day():
			...
			# upload to internet
			r = uploader.upload(file=camera.get_as_upload_payload(), file_len=camera.get_firmware_size(), version=camera.Version, release_notes=camera.ReleaseNotes, model_string= camera.ModelString, name=camera.Name, release_date=camera.ReleaseDate)
